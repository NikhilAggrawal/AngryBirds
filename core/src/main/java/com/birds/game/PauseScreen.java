...
public class PauseScreen implements Screen {
    ...
    public PauseScreen(AngryBirds1 game, GameScreen gameScreen) {
        ...
        if (AngryBirds1.soundOn) {
            soundbutton = new Button(new TextureRegionDrawable(new TextureRegion(new Texture("soundon.png"))));
        }else{
            soundbutton = new Button(new TextureRegionDrawable(new TextureRegion(new Texture("soundoff.png"))));
        }
        ...
        Button finalSoundbutton = soundbutton;
        soundbutton.addListener(new ClickListener() {
            @Override
            public void clicked(InputEvent event, float x, float y) {
                finalSoundbutton.addAction(Actions.sequence(
                ...
                if (AngryBirds1.soundOn) {
                    ...
                    finalSoundbutton.getStyle().up = new TextureRegionDrawable(new Texture(Gdx.files.internal("soundoff.png")));
                } else {
                    AngryBirds1.soundOn = true;
                    game.backgroundMusic.play();
                    finalSoundbutton.getStyle().up = new TextureRegionDrawable(new TextureRegion(new Texture("soundon.png")));
            ...
            public void enter(InputEvent event, float x, float y, int pointer, com.badlogic.gdx.scenes.scene2d.Actor fromActor) {
                finalSoundbutton.addAction(Actions.scaleTo(1.1f, 1.1f, 0.1f));
            }

            @Override
            public void exit(InputEvent event, float x, float y, int pointer, com.badlogic.gdx.scenes.scene2d.Actor toActor) {
                finalSoundbutton.addAction(Actions.scaleTo(1f, 1f, 0.1f));
            }
        });
